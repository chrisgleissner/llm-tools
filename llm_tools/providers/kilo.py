...
def _run_kilo_stats(env: dict[str, str]) -> dict[str, Any] | None:
    ...
    # *month-to-date*, not the all-time cumulative total. ``kilo stats``
    # defaults to all-time; without ``--days N`` a long-running install would
    # surface its full lifetime spend as if it were this month's bill.
...
def _scopes_for_mode(
    ...
    *,
    ...
    # (see ``_run_kilo_stats``), so the row is tagged with the next
