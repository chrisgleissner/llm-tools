...
* Copilot surfaces two rows: a `monthly` row for the included credit allowance (parsed from the CLI footer `Plan: N% used`) and a `balance` row showing additional ("add-on") usage as `spent $X` — the dollar amount billed beyond the allowance. The CLI cannot report the add-on figure, so it comes from the GitHub billing REST API (`/users/{login}/settings/billing/usage`, summing the current month's `netAmount` for `product == "copilot"`). It reuses an already-present GitHub token (`COPILOT_GITHUB_TOKEN` > `GH_TOKEN` > `GITHUB_TOKEN`, then `gh auth token`) so no Copilot-specific credential is ever required; with no token the `balance` row is omitted, never fabricated. The add-on read is isolated and cached (`LLM_USAGE_COPILOT_ADDON_TTL`): it rides in `model_scopes` (display-only) so it never gates provider readiness, and a transient billing-API failure degrades to the last cached figure. The `$5` budget *limit* is web-UI-only (no REST API exposes it), so only the spent amount is shown.
...
* `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` / `GITHUB_TOKEN` (GitHub token the add-on reader reuses, in this precedence; falls back to `gh auth token`. Never required — absent a token the add-on row is simply omitted)
