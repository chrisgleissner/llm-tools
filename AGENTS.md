...
Tests should use the env-var fallback path (`LLM_USAGE_KILO_*`) and `LLM_USAGE_COPILOT_CAPTURE_TEXT`-style fixture overrides rather than live `kilo stats` capture. The reader tries `kilo stats` first (JSON or human-readable) — bound to the current calendar month via `kilo stats --days <days_since_month_start>` so the surfaced cost is month-to-date, not the all-time cumulative total — and falls back to:
...
* `llm-usage` renders route rows using the launch CLI in the Provider column and the route's pinned model in the Model column; the route id remains internal/JSON metadata. No progress bar is rendered for opaque / fixed-subscription rows.
* For Kilo routes only, the Model column strips a `<vendor>-coding-plan/` prefix down to `coding-plan/` (e.g. `minimax-coding-plan/MiniMax-M3` → `coding-plan/MiniMax-M3`, via `usage._shorten_kilo_model_label`). Kilo sells these gateway models simply as "Coding Plan"; the vendor prefix is Kilo's internal namespacing, not a user-facing name, and only eats into the narrow column. Other providers' model ids are untouched.
* A route's `model` is the launch model ralph-robin pins via `--model`. `ralph_robin.effective_model_for` reads the route's model in route mode (`route_id` resolved in `cfg.route_policies`); it does **not** read `cfg.policies` there (which is empty in route mode). This is what lets two routes sharing the `kilo` launch CLI select `minimax-m3` vs `zai/glm-5.2` — without it kilo would launch with no `--model` flag and fall back to its configured default model.
* `--routes` (and `[ralph].routes`) accept a mix of declared route ids **and** bare provider names. A bare provider resolves to an implicit route on its own CLI via `routes.route_or_implicit_provider` (declared route wins on id collision). This lets a rotation mix plain CLI launches with model-pinned routes — e.g. `--routes codex,claude,kilo-minimax-m3,kilo-zai-glm-52` burns down four independent capacity pools where the two kilo routes share one launch CLI. An id that is neither a declared route nor a known provider is a hard configuration error.
...
* Treat a provider launch as needing retry on non-zero exit, or on a clean exit whose output clearly signals a provider rate-limit/overload. Keep `output_is_retryable` patterns specific so ordinary successful agent output is not re-submitted. The synthetic autonomy-abort status `75` is different: do not retry the same provider session inside `llm-scheduler`. When status `75` lands on a route (idle timeout, blocked prompt, hung tool call, or an opaque provider whose backend silently 429s), `submit_once` persists a route runtime block with `reason="autonomy-abort"` and the standard `LLM_TOOLS_LOCAL_BLOCK_BACKOFF` window so ralph-robin rotates away from the failing route instead of burning another full idle window on it. The block is honoured by the opaque-route decision path (`routes.is_locally_blocked`) and a successful subsequent run clears it.
...
* `LLM_USAGE_COPILOT_QUOTA_JSON` (inject a `/copilot_internal/user` quota-snapshot payload, bypassing all network access; used by tests for the primary monthly source)
